    ...
    @shopifystoreid = 98098
         ...
         @shopifycustomersinceid = topid.shopify_customer_id
           ...
           if @customercount > 0
              ...
              while @page <= @noofpages
                @customers.blank? ? @customers = ShopifyAPI::Customer.find(:all, :params => {:limit => 250, :page => 1, :since_id => @shopifycustomersinceid, :status => "any", :fields => "created_at,id,first_name,last_name,id,state,updated_at,adresses,tags" }) : @customers += ShopifyAPI::Customer.find(:all, :params => {:limit => 250, :since_id => @shopifycustomersinceid, :page => @page, :status => "any", :fields => "created_at,id,first_name,last_name,id,state,updated_at,adresses,tags" })
                @page += 1
              end
          end   
     ...
     if @customers  ###  UPDATE DATABASE WITH CUSTOMERS
        @customers.each do |shop_customer| 
            @existing_customer = Customer.where(:shopify_customer_id => shop_customer.id, :shopifystores_id => @shopifystoreid)
            if @existing_customer.blank?
 
                  shop_customer.addresses.each do |addresses| 
